/*
 * NFA2DFA2VEC.h
 * 将NFA转换为DFA，并对DFA进行最小化处理，最后将DFA存储为数组形式，用于生成.cpp文件
 * 优化了代码逻辑，重新组织了代码结构，增加了详细的注释
 */
...
/*
 * 构造子集
 * @param originSet：初始的NFA状态子集
 * @param constructedSet：构造出的NFA状态子集
 * @param edge：边对应的字符
 * @return：true--该集合有发出edge边，false--该集合没有发出edge边
 */
...
/*
 * 确定终态对应的动作
 * @param nfaStatesInDFAState：DFA状态包含的NFA状态
 * @param actions：DFA终态对应动作
 * @return：true--该DFA状态为终态，false--该DFA状态不是终态
 */
...
/*
 * 拆分DFA状态集合
 * @return：true--有集合被拆分，false--没有集合被拆分
 */
bool NFA2DFA2VEC::split_loop()
{
    bool flag = false;		   // 是否有集合被拆分
